      ...
      <div className="mt-8 space-y-3">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex w-full items-center justify-between rounded-xl border border-white/5 bg-black/50 px-6 py-4">
            <span className="text-glow-cyan font-mono text-4xl text-[#00E5FF]">
        ...
        <div className="rounded-xl border border-white/5 bg-black/50 px-6 py-5">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-glow-cyan mt-2 font-mono text-3xl text-[#00E5FF]">
                {(module.minBatchL ?? 0).toFixed(1)}{" "}
                <span className="text-base text-white/35">L</span>
              </p>
            </div>
...
function GhostModuleCard() {
  return (
    <motion.div
      ...
      whileTap={{ scale: 0.99 }}
      className="flex cursor-pointer items-center justify-center rounded-[2rem] border-2 border-dashed border-white/20 p-8 transition-all hover:border-cyan-500/50 hover:bg-cyan-900/10"
    >
          ...
          <section className="space-y-5">
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
              {displayModules.map((module, index) => (
                ...
                >
                  <DisplayModuleCard module={module} />
                </motion.div>
              ))}
              <GhostModuleCard />
            </div>
            ...
            <div className="grid grid-cols-1 gap-8 lg:grid-cols-2">
              {productionModules.map((module, index) => (
                <motion.div
                  ...
                  transition={{ duration: 0.4, delay: index * 0.05, ease: "easeOut" }}
                >
                  <ProductionModuleCard module={module} />
                </motion.div>
              ))}
              <GhostModuleCard />
            </div>
